...
export function buildExtraBadges(settings: GraphTechSettings): ExtraBadge[] {
    const owned = getTrueOwnedBadgeIds(UserStore.getCurrentUser()?.id);

    const extra: ExtraBadge[] = [];

    for (const [badgeId, shouldShow] of Object.entries(settings.store.badgeOverrides ?? {})) {
    ...
    for (const custom of settings.store.customBadges ?? []) {
